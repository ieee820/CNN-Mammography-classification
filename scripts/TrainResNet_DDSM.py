...
# model dimensions
image_height = 128#int(2768)
image_width  = 128#int(4664)
num_channels = 3  # RGB
num_classes  = 2
...
def train_and_evaluate(reader_train, reader_test, network_name, max_epochs):
    ...
    epoch_size = 512#512                    # for now we manually specify epoch size
...
if __name__=='__main__':
    ...
    reader_train = create_reader(os.path.join(data_path, 'training_set'), os.path.join(data_path, 'training_set'), True)
    reader_test  = create_reader(os.path.join(data_path, 'test_set'), os.path.join(data_path, 'test_set'), False)
